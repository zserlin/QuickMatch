...
# The line is no longer needed due to upgrade of numpy
...
def cal_density(features):
    ...
    gaus = np.exp((-0.5) * np.power(D_corrected,2))
...
def count_QM_matches(features, query_idx):
    ...
    for i in range(0, len(features.images)):
        ...
        im_idx2 = i
        if match.shape[0] == 0:
            # it is a situation that we should not encounter
            print("ERROR! Image does not match anything!")
        else:
